...
4. **Write a batch spec JSON.** Schema is in `execution/generate_gemini_ad_batch.py`'s docstring; `execution/ai_runner_gemini_batch.json` is a working 10-ad example. One entry per ad: `id`, `style_ref`, `aspect_ratio`, `scene`, `headline`, `subline`, `cta`, `copy_placement`. Rotate style refs and vary scene/lighting/time-of-day across the batch, and mix placements (feed 4:5, square 1:1, vertical 9:16).

   Keep headline copy in **short, common words** — that is what renders reliably.

5. **Start with 3 for approval unless the user has already approved a larger run.** Each attempt costs money; a systematic prompt flaw discovered at ad 30 is an expensive mistake.
...
Offer the hybrid path: generate imagery only (no copy in the prompt, negative space reserved), then overlay text with `execution/render_ads_to_png.py`. Perfect typesetting, brand fonts, and copy swaps without re-paying for images.

## Daily automation
A LaunchAgent runs `execution/run_daily_image_ads.py` at 8:00 AM every day: 5 ads picked statelessly from the 20-scene pool, uploaded to a dated Drive folder. See the directive's "Automation" section.

If the user asks to change the schedule, the count, or the scenes:
- Scenes/copy live in `execution/image_ad_scene_pool.json` — edit there, then **verify with `--dry-run`** (free) rather than a live run.
...
- Schedule lives in `~/Library/LaunchAgents/com.marketingcoursedemo.image-ads-daily.plist`; reload with `launchctl bootout` then `bootstrap`.
...
- `execution/generate_gemini_ad_batch.py` — batch runner (`--only` regenerates single ads)
- `execution/gemini_image_generate.py` — single-image function + CLI, wraps the Gemini call
- `execution/run_daily_image_ads.py` — unattended 8 AM daily batch (`--dry-run`, `--date`)
- `execution/image_ad_scene_pool.json` — 20-scene rotation pool
- `execution/ai_runner_gemini_batch.json` — working 10-ad example spec
- `execution/reference_images/` — product photo + style references (durable, not `.tmp/`)
